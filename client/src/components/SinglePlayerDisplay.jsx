...
class SinglePlayerDisplay extends React.Component {
  ...
  getPlayerGameLogs() {
    axios.get('/playergames',{params: {playerID: this.props.match.params.playerID}})
    .then((response) => {
  ...
  getPlayerSeasonStats() {
    axios.get('/playerseason',{params: {playerID: this.props.match.params.playerID}})
    .then((response) => {
  ...
  getPlayer() {
    axios.get('/player', {params: {playerID: this.props.match.params.playerID}})
    .then((response) => {
  ...
  statClickHandler(e) {
    ...
    this.setState({selectedStat: statName}, () => {this.generateGraph(this.state.selectedStat)});
  }
  ...
  generateGraph(statName = 'PtsPerGame') {
    var last10games = this.state.playerGameLogs.slice(-10);
    var yLabel;
    ...
    return (
      <div className ='center'>
        <Paper style={spdPaperStyle}>
          <PlayerBioBox teamName={this.props.match.params.teamName} playerID= {this.props.match.params.playerID} playerSeasonStats={this.state.playerSeasonStats} player={this.state.player}/>
          <div className = "statGraphContainer">
            <div className = "box1">
              <PlayerBioTable generateGraph={this.generateGraph.bind(this)} getSelectedStat={this.getSelectedStat.bind(this)} playerSeasonStats={this.state.playerSeasonStats}/>
            </div>
            <div className = "box2" id="chart"></div>
          </div>
        </Paper>
      </div>
    )
